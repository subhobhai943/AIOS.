...
def build_image(pairs: list[tuple[str, str]], output_path: str) -> None:
    if not pairs:
    ...
    print(f"[mkinitrd] wrote {output_path}  ({total} bytes, {n} file(s))")
    for i, (hp, rn) in enumerate(pairs):
        print(f"           {rn:40s}  {len(file_data[i])} bytes  <- {hp}")
