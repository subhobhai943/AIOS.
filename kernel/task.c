...
task_t *task_create(void (*entry)(void), uint32_t stack_size, const char *name)
{
    ...
    klog("[TASK] created PID "); klog_dec(t->pid);
    klog(" name="); klog(t->name);
    klog(" stack="); klog_hex((uint64_t)(uintptr_t)stack);
    klog(" rsp=");   klog_hex(t->rsp);
    klog("\r\n");
...
void task_destroy(task_t *t)
{
    ...
    if (t->stack_base) { kfree(t->stack_base); t->stack_base = NULL; }
    t->state = TASK_DEAD;
    klog("[TASK] destroyed PID "); klog_dec(t->pid); klog("\r\n");
}
...
/* ----------------------------------------------------------------
 * task_foreach — iterate all live task slots
 * ---------------------------------------------------------------- */
