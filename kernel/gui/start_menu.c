...
void start_menu_draw(framebuffer_t *fb, const gui_font_t *font)
{
    ...
    g_menu_h = START_MENU_PAD * 2u + item_count() * START_MENU_ITEM_H;
    if (fb->height > TASKBAR_HEIGHT + g_menu_h) {
        g_menu_y = fb->height - TASKBAR_HEIGHT - g_menu_h;
    } else {
        g_menu_y = 0;
    }

    fb_fill_rect(g_menu_x, g_menu_y, START_MENU_W, g_menu_h, UI_COLOR_WINDOW_BG);
    fb_draw_rect(g_menu_x, g_menu_y, START_MENU_W, g_menu_h, UI_COLOR_ACCENT);

    if (!font) return;

    for (uint32_t i = 0; i < item_count(); i++) {
        uint32_t item_y = g_menu_y + START_MENU_PAD + i * START_MENU_ITEM_H;
        fb_fill_rect(g_menu_x + START_MENU_PAD,
                     item_y,
        ...
        font_draw_string(fb,
                         ...
                         item_y + (START_MENU_ITEM_H - font->height) / 2u,
                         g_items[i].label,
                         UI_COLOR_TEXT_FG,
                         UI_COLOR_WINDOW_TITLE_INACTIVE_BG);
    }
